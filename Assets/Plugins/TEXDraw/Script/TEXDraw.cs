...
public class TEXDraw : MaskableGraphic, ITEXDraw, ILayoutElement, ILayoutSelfController
{
    ...
    public virtual Rect scrollArea
    {
        ...
        set
        {
            ...
            {
                m_ScrollArea = value;
                if (orchestrator?.InputCanvasSize(rectTransform.rect, m_ScrollArea, m_Padding, m_Overflow) ?? false)
                    m_BoxDirty = true;
                m_OutputDirty = true;
                SetVerticesDirty();
            }
    ...
    internal void CheckGraphicsDirty()
    {
        ...
        if (m_TextDirty)
        {
            ...
            if (!m_BoxDirty && orchestrator.InputCanvasSize(rectTransform.rect, m_ScrollArea, m_Padding, m_Overflow))
                m_BoxDirty = true;
        ...
        if (m_BoxDirty)
        {
            orchestrator.InputCanvasSize(rectTransform.rect, m_ScrollArea, m_Padding, m_Overflow);
            orchestrator.Box();
            m_OutputDirty = true;
            m_BoxDirty = false;
        }
