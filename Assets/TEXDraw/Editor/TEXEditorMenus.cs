...
namespace TexDrawLib.Editor
{
    ...
    public static class TEXEditorMenus
    {
...
#if TEXDRAW_TMP
        [MenuItem("Tools/TEXDraw/Rebuild TMP Atlases", false, 7)]
        public static void ImportAtlases()
        {
            FileUtil.DeleteFileOrDirectory(TEXPreference.main.MainFolderPath + "/Fonts/TMPro");
            FileUtil.DeleteFileOrDirectory(TEXPreference.main.MainFolderPath + "/Fonts/TMPro.meta");
        ...
        [MenuItem("Tools/TEXDraw/Invalidate TEXDraw on Scene", false, 7)]
        ...
        public static void SetTextMeshProIntegration()
        {
            ...
            else
            {
                ...
                var confirm = EditorUtility.DisplayDialog("Add TEXDRAW_TMP integration for all platform",
        ...
        public static void SetInputSystemIntegration()
        {
            ...
            else
            {
                ...
                EditorUtility.DisplayDialog("Operation Rejected", "You need to Install and Enable Input System through Unity Package Manager.", "OK");
                return;
#else
        ...
        static void PatchRuntimeAsmDef(bool activate)
        {
            // GUID of TEXDraw Runtime
            var path = AssetDatabase.GUIDToAssetPath("880e668efa4b7476bbac38252bba4224");
            if (path == "")
            {
                EditorUtility.DisplayDialog("Patch Runtime AsmDef Failed", "Can't find TEXDraw AsmDef Runtime. Need to patch yourself!", "OK");
                return;
            }
            var oldtext = File.ReadAllText(path);
            string newtext;
            var disableState = "    \"references\": [\n";
            // GUID of Unity.InputSystem
            var enableState = disableState + "        \"GUID:75469ad4d38634e559750d17036d5f7c\",\n";
            if (activate)
            {
                newtext = oldtext.Replace(disableState, enableState);
            }
            else
            {
                newtext = oldtext.Replace(enableState, disableState);
            }
            if (oldtext == newtext)
            {
                EditorUtility.DisplayDialog("Patch Runtime AsmDef Failed", "Can't apply patch to TEXDraw AsmDef Runtime. May need patch yourself?", "OK");
                return;
            }
