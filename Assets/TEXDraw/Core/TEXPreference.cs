...
namespace TexDrawLib.Core
{
    public partial class TEXPreference : ScriptableObject
    {
...
#if TEXDRAW_UIE
        private Material defaultMaterialUIE;


        public Material GetDefaultMaterialForUIE()
        {
            if (!defaultMaterialUIE)
                // The only thing that we can found at runtime build is in the Resource folder
                defaultMaterialUIE = (Material)Resources.Load("TEX-Default-UIE");
#if UNITY_EDITOR
            if (!defaultMaterialUIE)
            {
                //Create New One
                var shader = Shader.Find("Hidden/TEXDraw-UIE");
                defaultMaterialUIE = new Material(shader);
                if (AssetDatabase.IsValidFolder(DefaultTexFolder))
                {
                    AssetDatabase.CreateAsset(defaultMaterialUIE, DefaultTexFolder + "/Resources/TEX-Default-UIE.mat");
                }
                else
                {
                    //Find alternative path to the TEXPreference, that's it: Parent path of TEXPreference script.
                    string AlternativePath = AssetDatabase.GetAssetPath(MonoScript.FromScriptableObject(main));
                    AlternativePath = Directory.GetParent(AlternativePath).Parent.FullName;
                    AssetDatabase.CreateAsset(defaultMaterialUIE, AlternativePath + "/Resources/TEX-Default-UIE.mat");
                }
            }
#endif
             return defaultMaterialUIE;
        }
        ...
        public TextAsset hyphenationTexFile;

        private TexHyphenation hyphenationEngine;
        ...
        public TexHyphenation GetHyphenationEngine()
        {
            if (hyphenationTexFile != null && hyphenationEngine == null)
            {
                hyphenationEngine = new TexHyphenation(hyphenationTexFile);
            }
            return hyphenationEngine;
        }

        public TexAsset GetFontByID(string id)
        {
            return fontnames.TryGetValue(id, out TexAsset val) && val ? val : null;
        }

        public int GetFontIndexByID(string id)
        {
            return fontnames.TryGetValue(id, out TexAsset f) && f ? f.assetIndex : -1;
        }
